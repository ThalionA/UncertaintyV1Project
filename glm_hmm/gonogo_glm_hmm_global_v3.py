...
NUM_ITERS_ANIMAL = 50   # Stage 2: Intermediate iterations for Animal
...
for K in CANDIDATE_K:
    ...
    global_models[K] = sort_global_model_by_engagement(best_model)
...
plt.plot(list(global_bics.keys()), list(global_bics.values()), marker='o', color='black')
plt.title("Model Selection: BIC vs Number of States")
...
for animal_id, (X_a, y_a) in animal_data.items():
    ...
    model_a.fit(y_a, inputs=X_a, method="em", num_iters=NUM_ITERS_ANIMAL, observations_mstep_kwargs=dict(l2_penalty=2), verbose=0)
...
if DO_SESSION_FITS:
    ...
    for (animal_id, session_id), (X_s, y_s) in session_data.items():
        ...
        model_s.fit(y_s, inputs=X_s, method="em", num_iters=NUM_ITERS_SESSION, observations_mstep_kwargs=dict(l2_penalty=2), verbose=0)
