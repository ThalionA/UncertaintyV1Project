...
LOSS_COLOR = {'PCA (evar-weighted)': '#e6550d', 'flat L2 (Brier)': '#3182bd',
              'KL': '#7b3294'}
...
def _fig_examples(decoded, Tte, out_dir, Htgt):
    ...
    for ax, tr in zip(axes, picks):
        ...
        for name, dec in decoded.items():
            ax.plot(x, dec[tr], color=LOSS_COLOR[name], lw=1.8, label=name)
        ax.set_xlabel('orientation bin')
        ax.set_title(f'trial {tr}', fontsize=9)
    axes[0].legend(frameon=False, fontsize=7.5)
    axes[0].set_ylabel('probability')
    fig.suptitle('Toy model — decoded posteriors vs broad target  '
                 '(PCA loss spikes; flat-L2 / KL stay broad)', y=1.04, fontsize=12)
...
def _fig_spectrum(decoded, Tte, pcs, evar, out_dir):
    ...
    for name, dec in decoded.items():
        Dp = dec @ pcs.T
        err = ((Dp - Tt) ** 2).mean(0)
        ax.semilogy(k, err + 1e-12, color=LOSS_COLOR[name], lw=2.0, marker='o',
                    ms=2.5, label=name)
    ...
    ax.set_title('Where the peakiness lives — per-PC error spectrum\n'
                 'PCA & KL match the high-evar location PCs; only PCA diverges on '
                 'the low-evar shape PCs (its loss weights them ~0)')
...
def _fig_sweeps(C, n_train, n_test, n_neurons, tune_width, target_width, epochs,
    ...
    for name in LOSSES:
        axes[0].plot(hiddens, [g[name] for g in g_h], color=LOSS_COLOR[name],
    ...
    for name in LOSSES:
        axes[1].plot(noises, [g[name] for g in g_n], color=LOSS_COLOR[name],
                     lw=2, marker='o', label=name)
    ...
    fig.suptitle('Toy model — over-sharpening (positive gap) only under the PCA loss, '
                 'and it grows with capacity & uncertainty', y=1.02, fontsize=12)
    fig.tight_layout()
