...
def _synthetic_trials(n_trials=200, T=10, n_neurons=50, n_cats=91,
    ...
    """Return ``(x, logits)`` for synthetic time-binned activity.

    ``x`` has shape ``(n_trials, T, n_neurons)``; ``logits`` is the same
    shape passed through a fixed random MLP, ``(n_trials, T, n_cats)``.
    """
    rng = np.random.default_rng(seed)
...
def fig_2class_geometry(out_dir, T=8, sigma=1.5, seed=4):
    ...
    sigmoid(mean(z_1-z_0)) lands at a single point; the SBC operation
    mean(sigmoid(z_t)) lands at a different point (between them by
    Jensen's inequality, with the offset growing with input spread).
    """
    rng = np.random.default_rng(seed)
    mu = 0.3
    ...
    p_of_mean = 1.0 / (1.0 + np.exp(-z.mean()))    # PPC
    ...
    ax1.plot(zz, 1.0 / (1.0 + np.exp(-zz)), color='0.4', lw=2.2,
              label='sigmoid (concave on negatives, convex on positives)')
    ax1.scatter(z, p_bin, s=70, color=SBC_C, zorder=5,
                 edgecolor='white', linewidth=1.2,
                 label=f'T={T} per-bin logits → sigmoids')
    ax1.scatter([z.mean()], [p_of_mean], s=180, color=PPC_C, marker='*',
                 zorder=6, edgecolor='white', linewidth=1.5,
                 label=r'PPC: sigmoid($\overline{z}$)')
    ax1.scatter([z.mean()], [p_mean], s=180, color=SBC_C, marker='s',
                 zorder=6, edgecolor='white', linewidth=1.5,
                 label=r'SBC: $\overline{\mathrm{sigmoid}(z)}$')
    # Visual annotation of the Jensen offset.
    ax1.annotate('', xy=(z.mean(), p_of_mean), xytext=(z.mean(), p_mean),
                  arrowprops=dict(arrowstyle='<->', color='0.2', lw=1.4))
    ax1.text(z.mean() + 0.25, 0.5 * (p_of_mean + p_mean),
              'Jensen\noffset', fontsize=11, color='0.2', fontweight='bold',
              bbox=dict(facecolor='#fff7d6', edgecolor='0.5', pad=2))
    ax1.set_xlabel(r'$z = z_1 - z_0$  (logit difference)')
    ax1.set_ylabel(r'$p_1 = \mathrm{sigmoid}(z)$')
    ax1.set_title('A. The two operations live on the sigmoid curve',
                   fontsize=12, fontweight='bold')
    ax1.grid(alpha=0.25)
    ax1.legend(fontsize=9, frameon=False, loc='lower right')

    # Right: same on the 1-D probability simplex (a line p in [0,1]).
    ax2.hlines(0.5, 0, 1, color='0.6', lw=2)
    ax2.scatter(p_bin, [0.5] * T, s=70, color=SBC_C, zorder=5,
                 edgecolor='white', linewidth=1.2, label='per-bin softmaxes')
    ax2.scatter([p_of_mean], [0.5], s=200, color=PPC_C, marker='*',
                 zorder=6, edgecolor='white', linewidth=1.5,
                 label=r'PPC: sigmoid($\overline{z}$)')
    ax2.scatter([p_mean], [0.5], s=200, color=SBC_C, marker='s',
                 zorder=6, edgecolor='white', linewidth=1.5,
                 label=r'SBC: $\overline{\mathrm{sigmoid}(z)}$')
    ax2.axvline(0.5, color='0.7', ls='--', lw=1.2)
    ax2.text(0.5, 0.7, 'centre (broadest)', ha='center', fontsize=9, color='0.45')
    ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
    ax2.set_xlabel('p(class = 1)')
    ax2.set_yticks([])
    ax2.set_title('B. Where the two estimates sit on the 1-D simplex',
                   fontsize=12, fontweight='bold')
    ax2.legend(fontsize=9, frameon=False, loc='lower center', ncol=1)

    fig.suptitle('Jensen on a 2-class problem: SBC is pulled toward the centre',
                  fontsize=13, fontweight='bold')
...
def fig_noise_drives(out_dir, sigmas=(0.0, 0.5, 1.0, 1.5, 2.0, 3.0),
                      n_trials=300, T=10, n_neurons=60, seed=0):
    """Sweep within-trial noise; show what changes for each pipeline."""
    H_ppc, H_sbc = [], []
    V_ppc, V_sbc = [], []
    for s in sigmas:
        _, logits = _synthetic_trials(
            n_trials=n_trials, T=T, n_neurons=n_neurons,
            sigma_signal=0.5, sigma_bin=s, seed=seed)
        z_mean = logits.mean(axis=1)                       # (N, n_cats)
        p_ppc = softmax(z_mean)                            # (N, n_cats)
        p_per_bin = softmax(logits, axis=-1)               # (N, T, n_cats)
        p_sbc = p_per_bin.mean(axis=1)                     # (N, n_cats)
    ...
    axes[0].plot(sigmas, H_ppc, '-o', color=PPC_C, lw=2.2, ms=7,
                  label='PPC (softmax of mean)')
    axes[0].plot(sigmas, H_sbc, '-s', color=SBC_C, lw=2.2, ms=7,
                  label='SBC (mean of softmaxes)')
    axes[0].set_xlabel(r'Within-trial noise $\sigma_{bin}$')
    axes[0].set_ylabel('Mean H(prediction)  (nats)')
    axes[0].set_title('A. SBC prediction broadens with noise; PPC barely moves',
                       fontsize=11, fontweight='bold')
    axes[0].grid(alpha=0.25)
    axes[0].legend(fontsize=10, frameon=False)

    # B: trial-to-trial variance.
    axes[1].plot(sigmas, V_ppc, '-o', color=PPC_C, lw=2.2, ms=7,
                  label='PPC')
    axes[1].plot(sigmas, V_sbc, '-s', color=SBC_C, lw=2.2, ms=7,
                  label='SBC')
    axes[1].set_xlabel(r'Within-trial noise $\sigma_{bin}$')
    axes[1].set_ylabel('Trial-to-trial variance of prediction\n'
                        r'($\sum_c \mathrm{var}_n(p_n[c])$)')
    axes[1].set_title('B. SBC variance shrinks with noise; PPC unchanged',
                       fontsize=11, fontweight='bold')
    axes[1].grid(alpha=0.25)
    axes[1].legend(fontsize=10, frameon=False)

    fig.suptitle('Within-trial noise is the smoothing dial (SBC sees it, PPC doesn\'t)',
...
def fig_T_law(out_dir, T_grid=(1, 2, 4, 8, 16, 32, 64),
               n_trials=400, n_neurons=60, sigma_bin=1.0, seed=0):
    """Trial-to-trial variance of SBC shrinks as T grows; PPC is flat.

    The asymptotic SBC limit is the *deterministic* function
    ``E_noise[softmax(MLP(mean + ε))]`` of the trial mean — a smoothed
    function with zero Monte-Carlo variance.
    """
    V_ppc, V_sbc = [], []
    for T in T_grid:
        _, logits = _synthetic_trials(
        ...
        z_mean = logits.mean(axis=1)
        p_ppc = softmax(z_mean)
        p_per_bin = softmax(logits, axis=-1)
        p_sbc = p_per_bin.mean(axis=1)
        V_ppc.append(p_ppc.var(axis=0).sum())
        V_sbc.append(p_sbc.var(axis=0).sum())
    ...
    ax.plot(T_arr, V_ppc, '-o', color=PPC_C, lw=2.4, ms=8,
             label='PPC  (flat in T — only sees the mean)')
    ax.plot(T_arr, V_sbc, '-s', color=SBC_C, lw=2.4, ms=8,
             label='SBC  (Monte-Carlo over T softmaxes)')
    # Show a 1/T reference line anchored at T=1's SBC.
    ref = V_sbc[0] / T_arr * T_arr[0]
    ax.plot(T_arr, ref, '--', color=SBC_C, lw=1.4, alpha=0.55,
    ...
    ax.set_title('SBC is a Monte-Carlo estimator: variance falls as 1/T',
                  fontsize=12.5, fontweight='bold')
