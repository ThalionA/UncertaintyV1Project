function code = NewExperimentTheo25_full2_v3
...
%   - Full: a small block of “easy” stimuli first, then all 9 orientations × 9 (contrast, dispersion) pairs,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function vr = initializationCodeFun(vr)
        ...
        % The 9 (contrast, dispersion) pairs
        % !! MODIFIED to match createVRworlds_auto.m !!
        pairs = [ ...
            1    5 ;   % Easy pair
            1   30 ;
            1   90 ;
            ...
            0.01 5 ;
            0.01 45  ];
        cfg.fullPairs = pairs;   % 9x2 array: [contrast, dispersion]
        ...
        switch sessionType
            case 'Basic'
                % ------------------------------------------------------------
                % BASIC session:
                %   - Only orientations {0°, 90°}, dispersion=5°, contrast=1
                ...
                trialDispersions(:) = 5;
                trialContrasts(:)   = cfg.maxContrast5; % This is now 1
            ...
            case 'Full'
                ...
                % Steps:
                %   1) Ask number of easy trials (0° or 90°, 1 contrast, 5° dispersion).
                %   2) Generate those N_easy “easy” trials.
                %   3) Generate the remaining trials from all 9×9 grid (geometric bias).
                ...
                answerEasy = inputdlg({'Number of initial easy trials (0 or 90° only):'}, ...
                                'Full Session: Easy Trials', [1 30], {'50'});
                ...
                easyCont = cfg.maxContrast5 * ones(N_easy,1); % Will use 1
                ...
                % 3) Build the “remainder” block from all 9 orientations × 9 pairs
                ...
                % Geometric weights over 9 pairs
                kPairs = size(cfg.fullPairs,1);  % 9
