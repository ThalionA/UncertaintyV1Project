function code = NewExperimentTheo25_c50
% NewExperimentTheo25   Code for the ViRMEn experiment.
%   This version implements:
%     1. Oriented gratings with varying contrasts are now shown through ViRMEn.
%        - World 1: corridor.
%        - World 2: dark world (timeout).
%        - Worlds 3-38: grating worlds, where each of 9 orientations is paired with 4 contrast levels.
%          The worlds are arranged as:
%              Worlds 3-11: 100% contrast,
%              Worlds 12-20: 50% contrast,
%              Worlds 21-29: 25% contrast,
%              Worlds 30-38: 1% contrast.
%     2. Session types determine which grating orientations are available:
%           Basic Learning: [0, 90] (100% contrast only).
...
%     4. For Orientation Learning sessions, the initial "easy" trials use only 100% contrast,
%        while the remaining trials use a geometric probability distribution over contrasts,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function vr = initializationCodeFun(vr)
        ...
        prompt = {'Animal ID', 'Session length (minutes)', 'Number of trials', ...
            ...
            'Contrast scalar factor (0-1, lower biases towards 100% contrast)'};
        ...
        cfg.contrastScalar        = str2double(answer{14});
        ...
        % Each of 9 orientations is paired with 4 contrast levels:
        %   Worlds 3-11: 100% contrast,
        %   Worlds 12-20: 50% contrast,
        %   Worlds 21-29: 25% contrast,
        %   Worlds 30-38: 1% contrast.
        ...
        contrastLevels = [1, 0.5, 0.25, 0.01];
        cfg.contrastLevels = contrastLevels;

        %% Session Type Selection and Allowed Orientations
        % --- Session Type Selection and Allowed Contrasts ---
        sessionTypes = {'Basic Learning', 'Orientation Learning 1', ...
            'Orientation Learning 2', 'Orientation Learning 3', ...
            'Contrast Learning 1', 'Contrast Learning 2', 'Complete'};
        ...
        switch sessionType
            case 'Basic Learning'
                ...
                allowedContrastLevels = 1;  % 100% contrast only
            case 'Orientation Learning 1'
                allowedOrientations = [0, 15, 75, 90];
                allowedContrastLevels = 1;
            ...
            case 'Orientation Learning 3'
                ...
                allowedContrastLevels = 1;
            case {'Contrast Learning 1', 'Contrast Learning 2'}
                allowedOrientations = [0, 15, 30, 40, 45, 50, 60, 75, 90];
                if strcmp(sessionType, 'Contrast Learning 1')
                    allowedContrastLevels = [1, 0.5, 0.25];   % Exclude 1% contrast
                else % 'Contrast Learning 2'
                    allowedContrastLevels = [1, 0.5, 0.25, 0.01];  % Include all contrasts
                end
            case 'Complete'
        ...
        cfg.allowedContrastLevels = allowedContrastLevels;

        % Map allowed orientations to their corresponding grating world indices for 100% contrast.
        ...
        for ii = 1:length(allowedOrientations)
            ...
            % For 100% contrast, world index = idx + 2.
            allowedWorldIndices(ii) = idx + 2;
        end
        cfg.allowedWorldIndices = allowedWorldIndices;

        %% Create Trial Stimuli Based on Allowed Orientations and Contrasts
        % For Orientation Learning sessions, initial "easy" trials use only 100% contrast.
        % For remaining trials, contrasts are chosen from a geometric probability distribution.
        maxRun = 7;
        maxAttempts = 2e6;
        ...
        if strcmp(sessionType, 'Basic Learning')
            % In Basic Learning all stimuli come from [0,90] at 100% contrast.
            ...
            % For Basic Learning, all trials are at 100% contrast.
            cfg.trialContrasts = ones(cfg.numTrials, 1);
        else
            ...
            rewardedContrasts = zeros(remainingRewarded, 1);
            nonRewardedBlock = zeros(remainingNonRewarded, 1);
            nonRewardedFlag = false(remainingNonRewarded, 1);
            nonRewardedContrasts = zeros(remainingNonRewarded, 1);
            
            % Define weights for contrast selection using geometric distribution.
            weights = (cfg.contrastScalar).^(0:(length(cfg.allowedContrastLevels)-1));
            ...
            for i = 1:remainingRewarded
                ...
                r = rand;
                cumP = cumsum(weights);
                contrastIdx = find(r <= cumP, 1, 'first');
                rewardedContrasts(i) = cfg.allowedContrastLevels(contrastIdx);
            ...
            for i = 1:remainingNonRewarded
                ...
                cumP = cumsum(weights);
                contrastIdx = find(r <= cumP, 1, 'first');
                nonRewardedContrasts(i) = cfg.allowedContrastLevels(contrastIdx);
            end

            remainingTrialsStim = [rewardedBlock; nonRewardedBlock];
            remainingTrialsContrasts = [rewardedContrasts; nonRewardedContrasts];
            ...
            for attempt = 1:maxAttempts
                ...
                if validCandidate
                    ...
                    remainingTrialsContrasts = remainingTrialsContrasts(perm);
                    valid = true;
                    break;
                end
            end
            ...
            trialContrasts = [ones(cfg.numEasyTrials,1); remainingTrialsContrasts];
        ...
        switch vr.trialPhase
            case 'grating'
                if isempty(vr.gratingStartTime)
                    ...
                    % Determine stimulus orientation and contrast for this trial.
                    ...
                    contrastIdx = find(vr.cfg.contrastLevels == currentContrast, 1);
                    if isempty(contrastIdx)
                        error('Current contrast %.3f not found in contrast levels.', currentContrast);
                    end
                    % Map to world index: world index = 2 + (contrastIdx-1)*9 + idx.
                    gratingWorld = 2 + (contrastIdx - 1)*9 + idx;
