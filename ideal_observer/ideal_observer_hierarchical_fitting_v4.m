...
% - ADDED: Marginalized Likelihood and Decision Posterior extraction.
%
% --- Clean up workspace
% clear; close all; clc;
...
for i_animal = 1:numel(all_data)
    ...
    ani.data = struct( ...
        ...
        'conf_licks',  data.conf_licks(:), ... 
        'conf_vel',    data.conf_vel(:), ...   
    ...
    % Trialwise inferred uncertainties (Mapped to marginalized values)
    ...
    % --- NEW EXPORTS: Likelihood and Decision Posteriors ---
    ani.inferred.likelihood_marginal = inferred_uncertainties.lik_s_marginal;
    ani.inferred.decision_posterior  = inferred_uncertainties.decision_posterior;
...
function model_preds = calculate_model_predictions(trial_conditions_mat, params, utility, current_data)
    ...
    p_respond_all_conds = zeros(n_conds, 1); 
    binary_choice_all_conds = zeros(n_conds, 1); 
...
function inferred_unc = invert_model_for_single_trial_uncertainty(data, params, utility, base_spec)
    ...
    % --- NEW PREALLOCATIONS ---
    inferred_unc.lik_s_marginal    = nan(n_trials, n_s);
    inferred_unc.decision_posterior= nan(n_trials, 2);
    ...
    for i = 1:n_trials
        ...
        % =========================================================================
        % STEP 3: MARGINALISED Q(theta) & LIKELIHOOD
        % =========================================================================
        ...
        % --- ADDED: Marginalized Likelihood ---
        lik_s_m_i = maps_L_s_given_m{cond_idx};  
        L_marg = post_m * lik_s_m_i;             
        ...
        % --- ADDED: Decision Posterior ---
        inferred_unc.decision_posterior(i, :) = [p_go_marg, p_ngo_marg];
...
function data_out = align_data_to_go_by_side(data_in, go_side)
    ...
    switch side
        ...
        case {'vertical','v','>45','right'}
            flip_needed = true;
        otherwise
            error('align_data_to_go_by_side: unknown go side "%s".', go_side);
    end
    data_out = data_in;
    if flip_needed
