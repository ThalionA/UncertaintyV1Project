...
for i_animal = 1:numel(all_data)
    ...
    % Trialwise inferred uncertainties
    ani.inferred.perceptual     = inferred_uncertainties.perceptual(:);
    ani.inferred.decision       = inferred_uncertainties.decision(:);
    ...
    ani.inferred.eu_go          = inferred_uncertainties.eu_go(:);
    ani.inferred.eu_nogo        = inferred_uncertainties.eu_nogo(:);
...
% Note: Inversion uses licks + vel to infer m, ignoring choice
...
for j = 1:n_unique_conds
    ...
    % Perceptual Uncertainty (Circular Variance/Entropy proxy)
    ...
    % --- Decision Uncertainty (Entropy of Category Posterior) ---
    % Calculate P(Stimulus is Go | m)
    p_stim_go = sum(post_s_m(:, is_go_stim), 2) + 0.5 * sum(post_s_m(:, is_boundary), 2);
    p_stim_nogo = 1 - p_stim_go;
    
    % Entropy of the binary Go/NoGo distinction
    ...
    dv = eu_go - eu_nogo; % Still needed for behavioral mapping
...
n_trials = data.n_trials;
inferred_unc.perceptual = nan(n_trials, 1);
inferred_unc.decision = nan(n_trials, 1);
inferred_unc.m_posteriors = nan(n_trials, numel(m_range_rad));
inferred_unc.perceptual_map = nan(n_trials, 1);
inferred_unc.decision_map = nan(n_trials, 1);
inferred_unc.eu_go = nan(n_trials, 1);
inferred_unc.eu_nogo = nan(n_trials, 1);
inferred_unc.L_s_given_map = nan(n_trials, numel(s_range_rad));
inferred_unc.post_s_given_map = nan(n_trials, numel(s_range_rad));
...
for i = 1:n_trials
    ...
    inferred_unc.eu_go(i)          = maps_eu_go{cond_idx}(map_idx);
    inferred_unc.eu_nogo(i)        = maps_eu_nogo{cond_idx}(map_idx);
    inferred_unc.perceptual(i)     = sum(maps_perc_unc{cond_idx}' .* post_m);
    inferred_unc.decision(i)       = sum(maps_dec_unc{cond_idx}' .* post_m);
