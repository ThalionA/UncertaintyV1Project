...
def _build_cache(trials_list, state_list, stage1, grids) -> _Cache:
...
def _log_emiss_cached(cache: _Cache, seq_i: int,
                      p_go_cond: list[np.ndarray]) -> np.ndarray:
    """(T, K) Bernoulli log-emission for one sequence from cached P(go)."""
...
def _run_em(init: IOHMMParams, cache: _Cache,
    ...
    for _ in range(max_iters):
        ...
        for i in range(n_seq):
            log_emiss = _log_emiss_cached(cache, i, p_go_cond)
