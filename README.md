...
The codebase is organized modularly to separate experimental control, preprocessing, behavioral modeling, and neural decoding:

* **`experiment_code/`**: Custom ViRMEn-based VR engine (MATLAB) used for behavioral monitoring and rendering the linear corridor, alongside Arduino scripts for hardware control (rotary encoder, lick spout).
* **`preprocessing/`**: Pipeline for 2-photon calcium imaging data. Includes wrappers for NoRMCorre (motion correction), FISSA (neuropil subtraction), and CASCADE (spike deconvolution optimized for jGCaMP8). Also contains custom scripts for longitudinal ROI registration across days.
* **`glm_hmm/`**: Generalized Linear Model Hidden Markov Model implementation used to isolate engaged perceptual states from history-dependent or task-disengaged states.
* **`ideal_observer/`**: Generative behavioral modeling to extract trial-by-trial perceptual posteriors and decision uncertainty from continuous kinematic readouts (velocity and lick rates).
* **`nn_decoder/`**: The core PyTorch deep learning pipeline.
    * `neural_network_classifier_v26.py`: Defines the PPC and SBC neural network architectures and custom loss functions (KL, JS, Wasserstein, PCA-weighted Euclidean).
    * `experiment_manager.py`: Manages the combinatorial hyperparameter grid (temporal bins, entropy penalties).
    * `run_experiment_v26.py`: The main training loop.
    * `results_deepdive.py` & plotting scripts: Generate spatiotemporal predictive profiles and theoretical presentation figures.
